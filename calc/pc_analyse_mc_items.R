# the purpose of this file is to check whether some answer options should
# be excluded from the multiple choice items 
# TODO: also check  thresholds for points?
# For this purpose, the analysis uses the virtual person approach
# The dataset is need in which mutiple choice items were not yet scored as one item
library(readr)
library(TAM)
...
# as the data includes raw mc data, there are more items than actual items (42 in total)
items_pro = c("a1_1","a1_3","a1_5","a2_1","a2_3","a2_5","b1_1","b1_3","b1_5","b2_1","b2_3","b2_5")


Q <- array( 0, dim=c(ncol(data_vp),2))
for (i in 1:nrow(Q)) {
  if (substr(colnames(data_vp)[i],1,4) %in% items_pro) {
    # item is procedural
    Q[i,1] = 1
  } else {
    # item is conceptual
